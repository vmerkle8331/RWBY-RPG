...
{
    class Blake : BaseCombatant
    {
        public Blake(decimal hPIn, decimal attackIn, decimal defenseIn) //takes in data to make Blake
        ...
        public decimal Katana() //basic attack that doesn't use any of the variables and causes minimal damage
        {
            return DealDamage(AttackDamage);
        }
        public decimal Gun(GameWindow singlePlayerWindow, _2PlayerGameWindow twoPlayerWindow) //attack that uses the dust variable and causes more damage than stab if out of dust attack fails
        {
            if (amountOfDust > 0)
            {
                amountOfDust--;
            ...
            {// If the Blake doesn't have enough Dust, text is displayed to notify the player.
        ...
        public decimal Shadow(GameWindow singlePlayerWindow, _2PlayerGameWindow twoPlayerWindow)
        {
            ...
            {// If the Blake doesn't have enough Semblece Power, text is displayed to notify the player.
        ...
        public decimal DoAction(string actionName, GameData currentData, Nevermore nevermore, DeathStalker deathStalker, Boarbatusk boarbatusk, Beowolf beowolf, Yang yang, Weiss weiss, Ruby ruby, Blake blake, GameWindow singlePlayerWindow, _2PlayerGameWindow twoPlayerWindow)
        {
            ...
            switch (actionName)
            {
                ...
                case "Gun":
                    {
                        damageTotal = Gun(singlePlayerWindow, twoPlayerWindow);
                        return damageTotal;
                    }
                case "Shadow":
                    {
                        damageTotal = Shadow(singlePlayerWindow, twoPlayerWindow);
                        return damageTotal;
                    }
                default:
                    {
