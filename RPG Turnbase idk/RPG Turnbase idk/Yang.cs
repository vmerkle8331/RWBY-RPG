...
{
    class Yang : BaseCombatant
    {
        public Yang(decimal hPIn, decimal attackIn, decimal defenseIn) //takes in data to make Yang
        ...
        public decimal Gauntlets() //basic attack that doesn't use any of the variables and causes minimal damage
        {
            return DealDamage(AttackDamage);
        }
        public decimal Shotgun(GameWindow singlePlayerWindow, _2PlayerGameWindow twoPlayerWindow) //attack that uses the dust variable and causes more damage than stab if out of dust attack fails
        {
            if (amountOfDust > 0)
            {
                amountOfDust--;
            ...
            {// If the Yang doesn't have enough Dust, text is displayed to notify the player.
        ...
        public decimal Burn(GameWindow singlePlayerWindow, _2PlayerGameWindow twoPlayerWindow)
        {
            ...
            {// If the Yang doesn't have enough Semblece Power, text is displayed to notify the player.
        ...
        {
            //variable damageTotal is created to store the total amount of damage Yang will do this turn. 
            ...
            switch (actionName)
            {
                ...
                case "Gauntlets":
                    {
                        damageTotal = Gauntlets();
                        return damageTotal;
                    }
                case "Shotgun":
                    {
                        damageTotal = Shotgun(singlePlayerWindow, twoPlayerWindow);
                        return damageTotal;
                    }
                case "Burn":
                    {
                        damageTotal = Burn(singlePlayerWindow, twoPlayerWindow);
                        return damageTotal;
                    }
                default:
                    {
